...
// src/cli/command_loader.ts
class CommandLoader {
  static async discover(program2, baseDir) {
    const root = baseDir ?? process.cwd();
    await this.loadPackageCommands(program2, root);
    await this.loadUserCommands(program2, root);
  }
  static async loadPackageCommands(program2, root) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            if (typeof module.register === "function") {
              module.register(program2);
            }
          } catch (err) {
            console.error(source_default.yellow(`Warning: Failed to load command "${file}": ${err instanceof Error ? err.message : err}`));
          }
        }
      } catch {}
    }
  }
  static async loadUserCommands(program2, root) {
  ...
  static async resolvePackages(root) {
    ...
    if (dirExists(nodeModulesDir)) {
      ...
      for (const dir of dirs) {
        ...
        if (commandsDir)
          results.push({ root: pkgRoot, commandsDir });
      }
    }
    try {
      ...
      if (existsSync(rootPkgPath)) {
        ...
        for (const ws of workspaces) {
          ...
          if (commandsDir)
            results.push({ root: wsPath, commandsDir });
        }
      }
    } catch {}
    return results;
...
async function readStravCommands(packageRoot) {
  try {
...
// src/cli/strav.ts
var pkg = await Bun.file(join2(import.meta.dir, "../../package.json")).json();
var program2 = new Command;
program2.name("strav").description("Strav CLI").version(pkg.version);
await CommandLoader.discover(program2);
program2.parse();
